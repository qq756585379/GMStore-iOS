...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {//头部
        DCCommentHeaderCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[DCCommentHeaderCell cellReuseIdentifier]
                                                                              forIndexPath:indexPath];
        cell.comNum = @"668";
        cell.wellPer = @"100%";
        gridcell = cell;
    }else if (indexPath.section == 1){//中间
        DCCommentContentCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[DCCommentContentCell cellReuseIdentifier]
                                                                               forIndexPath:indexPath];
        cell.backgroundColor = [UIColor orangeColor];
        gridcell = cell;
    }else if (indexPath.section == 2){ //底部
        DCCommentFooterCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[DCCommentFooterCell cellReuseIdentifier]
                                                                              forIndexPath:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    return (indexPath.section == 0) ? CGSizeMake(SCREEN_WIDTH, 30)  : (indexPath.section == 2) ? CGSizeMake(SCREEN_WIDTH / 2, 40)  : CGSizeMake(SCREEN_WIDTH, 200);
}
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        //注册
        [_collectionView registerClass:[DCCommentHeaderCell class] forCellWithReuseIdentifier:[DCCommentHeaderCell cellReuseIdentifier]];
        [_collectionView registerClass:[DCCommentFooterCell class] forCellWithReuseIdentifier:[DCCommentFooterCell cellReuseIdentifier]];
        [_collectionView registerClass:[DCCommentContentCell class] forCellWithReuseIdentifier:[DCCommentContentCell cellReuseIdentifier]];
