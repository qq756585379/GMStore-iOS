...
/* 推荐商品数据 */
@property (strong , nonatomic)NSMutableArray<GMRecommendItem *> *detailRecItem;
/* 页面 */
...
- (void)setUpUI
{
    ...
    _detailRecItem = [GMRecommendItem mj_objectArrayWithFilename:@"DetailRecommend.plist"];
}
...
- (void)layoutSubviews
{
    ...
    _pageControl.frame = CGRectMake(self.width * 0.5 - 40, self.height - (DCMargin * 2), 80, 20);//指定位置大小
}
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        layout.itemSize = CGSizeMake(self.width / 3, self.width / 3 + 60);
        layout.scrollDirection = UICollectionViewScrollDirectionHorizontal; //滚动方向
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        [self addSubview:_collectionView];
        _collectionView.frame = CGRectMake(0, 0, self.width, self.height - 20);
